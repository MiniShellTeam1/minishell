...
static char	*strip_delimiter_quotes(char *delimiter)
{
	...
	if (len >= 2)
	{
		...
		else if (len >= 2 && delimiter[len - 1] == '\'' && delimiter[len
			- 2] == '\'')
		{
			// Handle lim'' case
...
static int	should_expand_heredoc(char *delimiter)
{
	...
	{
		if ((delimiter[0] == '\'' && delimiter[len - 1] == '\'')
			|| (delimiter[0] == '"' && delimiter[len - 1] == '"') || (len >= 2
				&& delimiter[len - 1] == '\'' && delimiter[len - 2] == '\''))
		{
...
static char	*process_heredoc_line(char *line, t_master *master,
		int should_expand)
{
	char	*result;
...
void	handle_heredoc(t_command *cmd, t_master *master)
{
	...
	while (1)
	{
		...
		if ((size_t)ft_strlen(line) == delim_len && ft_strncmp(line,
				clean_delimiter, delim_len) == 0)
		{
			free(line);
			break ;
		}
		...
		char *new_content = concat_heredoc_line(heredoc_content,
				processed_line);
		if (heredoc_content)
			free(heredoc_content);
