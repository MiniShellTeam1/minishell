...
static int	ft_checkdir(t_command *cmd)
{
	...
	if (!dir)
	{
		if (errno == ENOTDIR)
			...
			ft_printerror(cmd->args[0], cmd->args[1],
...
static int	ft_setpwd(t_master *master)
{
	...
	else
	{
		if (!ft_addvar(&master->env, ft_getstralloc("PWD"), cwd))
			return (1);
		/*         if (!ft_getvar(master->env, "PWD") || !ft_getvar(master->env,
				"PWD")->key || !ft_getvar(master->env, "PWD")->value)
					return (1); */
	}
	return (0);
}
...
static int	ft_setoldpwd(t_master *master, t_env *pwdvar)
{
	...
	else
	{
		if (!ft_addvar(&master->env, ft_getstralloc("OLDPWD"), oldpwdpath))
			return (1);
		/*         if (!ft_getvar(master->env, "OLDPWD")
			|| !ft_getvar(master->env, "OLDPWD")->key || !ft_getvar(master->env,
				"OLDPWD")->value)
					return (1); */
	}
	return (0);
